...
module.exports.Formatter = class {
    ...
    setConfiguration() {
        const cfg = workspace.getConfiguration('clojure');
    ...
    processToken() {
        ...
        if (this.isCloseToken(this.token)) {
        ...
        switch (this.token.type) {
            ...
            case types.OPEN_SQ_BRACKETS:
            case types.OPEN_CRLY_BRACKETS:
    ...
    isCloseToken(token) {
        return token.type === types.CLOSE_PARENS
            ...
            || token.type === types.CLOSE_CRLY_BRACKETS;
    }
    ...
    fixIndent(token, indent) {
        ...
        if (this.hasTabs(token)) {
            const start = new Position(orig.end.line, 0);
            const end = new Position(orig.end.line, orig.end.character);

            this.edits.push(TextEdit.delete(new Range(start, end)));
            this.edits.push(TextEdit.insert(end, ' '.repeat(indent)));
    ...
    stackCloseParens(count) {
        while (count > 0) {
            ...
            if (this.token.type === types.WHITE_SPACE) {
                this.deleteToken(this.token);
            } else if (this.isCloseToken(this.token)) {
                this.sexprs.pop();
                count -= 1;
            } else {
                break;
    ...
    findNextCloseParen() {
        for (let ndx = this.token.ndx + 1; ndx < this.tokens.length; ndx += 1) {
            if (this.isCloseToken(this.tokens[ndx])) {
